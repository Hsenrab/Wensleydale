...
while keep_running:
    try:
        ...
        MY_CYCLE = blocklightpatterns.ChangingBlockLightPattern(num_led=NUM_LED, pause_value=0.04, num_steps_per_cycle=700, num_cycles=1) 
        MY_CYCLE.start()
        
        print('Run Gromit Colours')
        MY_CYCLE = blocklightpatterns.GromitColours(num_led=NUM_LED, pause_value=0.04, num_steps_per_cycle=100, num_cycles=1) 
        MY_CYCLE.start()
